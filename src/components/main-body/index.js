...
const Indexpage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!docsId) {
      toast.error("Please Paste a Google Docs link."); // Display toast error if docsId is empty
      return;
    }
    convertToMarkdown(docsId); // Trigger conversion when form submitted
  };
  ...
  return (
    <main className="bg-white flex justify-center min-h-screen pt-16">
        ...
        <div className= " mx-auto">
          ...
          <div className="mt-4 ">
            <p className="text-center flex justify-center text-gray-700 mb-8 ">
              Effortlessly convert  Google Docs to
              markdown in just one click!
            </p>
          </div>
          <form
          ...
          {markdown && (
            <div>
              ...
              <button
                ...
                className="absolute top-0 right-0 bg-blue-600 text-white p-2 rounded-md mr-4 mt-4 hover:bg-purple-500 focus:outline-none"
              >
                <FaCopy />
              </button>
              <div className="mt-4 p-4 bg-gray-100 rounded-md">
                <textarea
                  value={markdown}
                  readOnly
                  className="w-full h-64 p-2 bg-white text-black border border-gray-300 rounded-md focus:outline-none focus:border-blue-500"
                />
              </div>
            </div>
          )}
